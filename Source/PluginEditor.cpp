    ...
    : AudioProcessorEditor (&p), audioProcessor (p), valueTreeState (vts)
{
    ...
    for (int i = 0; i < N_LINES; i++) 
    {
        ...
        addAndMakeVisible(modFreqLabels[i]);
        addAndMakeVisible(modFreqSliders[i]);
...
void ReverberatorAudioProcessorEditor::resized()
{
    ...
    for (int i = 0; i < N_LINES; i++) 
    {
        ...
        modFreqLabels[i].setBounds(305, i * (border + size), 100, size);
        modFreqSliders[i].setBounds(405, i * (border + size), 185, size);
